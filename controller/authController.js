...
exports.forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (user.googleId) {
      throw new Error(
        `Email ${user.email} dùng đăng nhập thông qua google , bạn hãy đăng nhập lại theo tài khoản google của bạn`
      );
    }
      ...
      Messages: [
        {
          ...
          HTMLPart: `<h3>Xin chào ${user.name},</h3><p>Bạn đã yêu cầu thay đổi mật khẩu trên MYWAYMYFASHION. Vui lòng truy cập trang đổi mật khẩu và thực hiện theo hướng dẫn để hoàn tất quá trình.<br /> <a href="https://myway-front.vercel.app/account/user/resetPassword/${resetToken}">Tại đây !</a> </p><p>Trân trọng,</p><p>MYWAYMYFASHION</p>`,
        },
      ],
    });
    res.status(200).json({
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      throw new Error(
        "Sử dụng mã mới , mã này đã  hết hạn để reset lại mật khẩu"
      );
    }
