...
const Detail: React.FC = (props) => {
  ...
  const addToCart = async (objCart: ITEM) => {
    if (handleLoginAndCart.token) {
      try {
        ...
        dispatch(
          handleNotify({ message: "Đã có lỗi xảy ra", show: true, status: 400 })
        );
        setTimeout(() => {
          dispatch(handleNotify({ message: "", show: false, status: 0 }));
        }, 2000);
  ...
  const createComment = async (productId: string, text: string) => {
    try {
      const res = await axios.post("/myway/api/reviews/", {
        product: productId,
        text,
      });
      if (res.data.status === "success") {
  ...
  const replyComment = async (idCmt: string, text: string) => {
    try {
      const res = await axios.post(`/myway/api/reviews/comments/${idCmt}`, {
        text,
      });
      if (res.data.status === "success") {
        setComments((prev) => {
  ...
  return (
    <div>
      ...
      <div
        className={`container-md ${styles.detailPage}`}
      ...
      >
          ...
          <div className={`row`}>
              ...
              <div className={`${styles.productDetailImageSlide}`}>
                <div>
                  <Slider {...settings} className="my-slider">
                    {prod?.quantity &&
                      prod.quantity.map((each, index) => {
                        return each.imageSlideShows.map((el, idx) => {
                          return (
                            <div
                              className={styles.carouselSlick}
                              key={idx}
                              onClick={(e) => setChangeImg(el)}
                            >
            ...
            <div className={`${styles.productDetailAll} col-lg-6 col-md-6`}>
              ...
              <p className={`${styles.price}`}>
                <del>{prod?.oldPrice.toLocaleString("vi-VN")}₫</del>{" "}
                <h2>{prod?.newPrice.toLocaleString("vi-VN")}₫</h2>
              </p>
              <p className={`${styles.status}`}>
                Tình trạng :{" "}
                {prod?.subQuantity && prod.subQuantity > 0 ? (
                  <p>Còn hàng</p>
                ) : (
                  <p>Hết hàng</p>
                )}
              ...
              <div className={`${styles.colorOrder}`}>
                ...
                <div className={styles.allColor}>
                  {prod?.quantity &&
                    prod.quantity.map((el, idx) => {
                      return (
                        <div
                          key={idx}
                          className={
                            handleCart.tabColor === idx
                              ? `${styles.setBorder}`
                              : `${styles.setUnBorder}`
                          }
                          onClick={(e) => {
                        ...
                        >
                          <div className={styles.colorOderImage}>
                            <img
                              src={`${prod.quantity[idx].imageSlideShows[0]}`}
                            />
                          </div>
                          <p>{el.colorName}</p>
                        </div>
              ...
              <div className={`${styles.size}`}>
                <p>Kích thước : </p>
                <div>
                  {prod.quantity[handleCart.tabColor]?.size.map(
                    (each, index) => {
                      if (each.quantity > 0) {
                        return (
                          <button
                            key={index}
                            className={
                              handleCart.tabSize === index
                                ? `${styles.setBorderButton}`
                                : `${styles.setUnBorderButton}`
                            }
                            onClick={(e) => {
                              dispatch(setTabSize(index));
                            }}
                          >
                            {each.size}
                          </button>
                  ...
                  <div>
                    <button
                      onClick={(e) => {
                        ...
                        if (
                          handleCart.tabSize &&
                          handleCart.orderQuantity <=
                            prod.quantity[handleCart.tabColor].size[
                              handleCart.tabSize
                            ].quantity
                        ) {
                          dispatch(setEnable());
              ...
              <div className="row">
                <button
                  className={`${styles.buying} col-lg-6 col-md-6 col-sm-6 col-6`}
                >
                  MUA NGAY
                  <br />
                  Giao hàng tận nơi
                ...
                <button
                  className={`${styles.adding} col-lg-6 col-md-6 col-sm-6 col-6`}
                  disabled={prod.subQuantity === 0}
                  onClick={(e) => {
                    if (handleCart.tabSize === null) {
                      alert("Vui lòng chọn size");
                      return;
                    ...
                    addToCart({
                      ...
                      size: prod.quantity[handleCart.tabColor].size[
                        handleCart.tabSize || 0
                      ].size,
                      image:
                        prod.quantity[handleCart.tabColor].imageSlideShows[0],
                    });
                    dispatch(defaultTab());
                  }}
                >
              ...
              <ul>
                ...
                <li>
                  ...
                  <strong>
                    MIỄN PHÍ VẬN CHUYỂN VỚI ĐƠN HÀNG THANH TOÁN ONLINE
                  </strong>
                </li>
                <li>
                  <div>
              ...
              <div
                className={`${styles.supportBuying}`}
                style={{ marginTop: "30px" }}
              >
                ...
                <div>
                  ...
                  <i>
                    Gọi ngay để được tư vấn miễn phí qua hotline hoặc Zalo số{" "}
                    <strong>0862.278.465</strong> (8:30 - 22:00).
                  </i>
                </div>
              </div>
            </div>
        ...
        <div className={`${styles.productReviews}`}>
          <Tabs isLazy>
            <TabList>
              <Tab _selected={{ color: "#ec1f27" }} className={styles.tabBtn}>
                Mô tả sản phẩm
              </Tab>
              <Tab _selected={{ color: "#ec1f27" }} className={styles.tabBtn}>
                Hướng dẫn chọn size
            ...
            <TabPanels>
              ...
              <TabPanel>
                <div className={`${styles.content1}`}>
                  <p>
                    Chất liệu Gấm nhập khẩu cao cấp đã qua xử lý và tạo form
                    dáng mềm mại, mang lại sự thoải mái cho thượng khách, toát
                    lên vẻ trẻ trung, phong cách hiện đại đầy đẳng cấp
                  </p>
                  <p>
                    Sản phẩm cần giặt khô/tay, tuyệt đối không sử dụng máy giặt,
                    không giặt chung đồ khác màu, tránh dính màu khác...
                  </p>
                  <p>
                    Không nên giặt sản phẩm với xà phòng có chất tẩy mạnh, nên
                    giặt cùng xà phòng pha loãng.
                  </p>
                  <p>
                    Phơi sản phẩm tại chỗ thoáng mát, tránh ánh nắng trực tiếp,
                    nên làm khô quần áo bằng gió sẽ giữ màu vải tốt hơn.
                  </p>
                  <p>
                    Là hơi với nhiệt độ phù hợp để giữ màu sản phẩm được đẹp và
                    bền lâu hơn
                  </p>
                </div>
              </TabPanel>
              ...
              <TabPanel>
                <div className={`${styles.content2}`}>
                  <strong>
                    Quý khách có thể tham khảo bảng thông số size quy chuẩn của
                    My Way để lựa chọn cho mình những sản phẩm có kích thước phù
                    hợp với số đo của mình:
                  </strong>
                  ...
                  <div className={`${styles.ownerComment}`}>
                    <div className={`${styles.quantityComment}`}>
                      ...
                      <p style={{ display: "inline-block", cursor: "pointer" }}>
                        <i
                          className="fa-light fa-bars-sort"
                      ...
                      <div className={styles.allComments}>
                        {comments &&
                          ...
                          comments.map((comment, idx) => {
                            return (
                              <div className={styles.eachComment} key={idx}>
                                <div className={`${styles.userCommentContent}`}>
                                  ...
                                  <div>
                                    ...
                                    <p>
                                      {moment(comment.createAt).format(
                                        "DD/MM/YYYY"
                                      )}
                                    </p>
                                  </div>
                                  {comment.user._id ===
                                    handleLoginAndCart.user._id && (
                                    ...
                                    <div style={{ marginBottom: "25px" }}>
                                      <div className={`${styles.inputComment}`}>
                                        ...
                                        <input
                                          value={resComment}
                                          onChange={(event) =>
                                            setResComment(event.target.value)
                                          }
                                        />
                                      </div>
                                      <button
                                ...
                                {comment.response &&
                                  ...
                                  comment.response.map((res, index) => {
                                    return (
                                      ...
                                      >
                                        <div
                                          className={`${styles.userCommentContent}`}
                                        >
                                          <Image
                                            borderRadius="full"
                                            boxSize="40px"
