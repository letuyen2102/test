...
const ChangeProduct = () => {
  ...
  const [imageSlideShows, setImageSlideShows] = useState<IMAGE_SHOW>({
    quantity: [],
  });
  const [imageSlideShowsFormdata, setImageSlideShowsFormData] =
    useState<IMAGE_SHOW_FORMDATA>({ quantity: [] });
      ...
      await fetch(`/myway/api/products/getProductById/${idProd}`)
        ...
        .then((all) => {
          setProd(all.product),
            ...
            setImageSlideShows({
              quantity: Array(all.product.quantity.length).fill({
                color: "",
                imageSlideShows: [],
              }),
            }),
            setImageSlideShowsFormData({
              quantity: Array(all.product.quantity.length).fill({
                color: "",
                imageSlideShows: [],
              }),
            ...
            <div className="col-lg-12">
              <div className={styles.action}>
                ...
                <div className="row">
                  ...
                  >
                    * Màu sắc : {prod.quantity.length} loại
                  </p>
                  {prod.quantity.length > 0 &&
                    prod.quantity.map((each, idx) => {
                      return (
                        <div key={idx}>
                          <div
                            style={{
                              borderBottom: "2px dashed #00b156",
                              margin: "10px 0px",
                            }}
                          >
                            <div className="row">
                              <div className="col-lg-6 col-md-6 col-sm-12">
                                <div className={styles.formGroup}>
                                  <label>Tên màu</label>
                                  <input
                                    value={prod.quantity[idx]?.colorName}
                                    onChange={(event) =>
                                      setProd((prev) => {
                                        const newState = { ...prev };
                                        newState.quantity[idx].colorName =
                                          event.target.value;
                                        return newState;
                                      })
                                    }
                              ...
                              <div className="col-lg-6 col-md-6 col-sm-12 col-12">
                                <div className={styles.formGroup}>
                                  ...
                                  <input
                                    ...
                                    value={
                                      each.size.find(
                                        (aa, ii) => aa.size === selectSize[idx]
                                      )?.quantity ?? ""
                                    }
                                    onChange={(event) => {
                                      setProd((prev) => {
                                        const newState = { ...prev };
                                        newState.quantity[idx].size.forEach(
                                          (aa, ii) => {
                            ...
                            <div className={styles.postImage}>
                              <div className={styles.imageSlideShow}>
                                <div className="row">
                                  ...
                                  >
                                    Ảnh slideshow màu {each.colorName}{" "}
                                    <label className={styles.btnUploadImage}>
                                      THÊM ẢNH
                                      <input
                                        type="file"
                                        multiple
                                    ...
                                    <button
                                      className={styles.btnDeleteColor}
                                      onClick={(event) => {
                                        setProd((prev) => {
                                          const newState = { ...prev };

                                          newState.quantity =
                                            newState.quantity.filter(
                                              (mm, nn) => {
                                                return nn !== idx;
                                              }
                                            );
                                  ...
                                  {imageSlideShows.quantity[
                                    idx
                                  ]?.imageSlideShows.map((ei, ii) => {
                                    return (
                                      <div key={ii} className="col-lg-2">
                                        <div>
                                          <img
                    ...
                    onClick={(event) => {
                      ...
                      imageSlideShowsFormdata.quantity.forEach((gg, hh) => {
                        gg.imageSlideShows.forEach((jj, kk) => {
                          formData.append(
                            `imageSlideShow${slugify(gg.colorName, {
                              locale: "vi",
                              lower: true,
                            })}`,
                            jj
